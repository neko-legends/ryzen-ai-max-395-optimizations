param(
    ...
    [int]$UBatchSize = 1024,
    [string]$ModelPath = "",
    [string[]]$ModelPatterns = @("Qwopus3.6-35B-A3B-Coder-MTP-Q5_K_M.gguf"),
    [string[]]$SearchRoots = @(
...
function Resolve-QwopusModel {
    ...
    if (-not $model) {
        ...
        throw "Could not find $patterns under: $roots. Run the Qwopus download/install BAT first or pass -ModelPath."
    }
...
$ResolvedModelPath = Resolve-QwopusModel

Write-Host "Starting Qwopus3.6 35B-A3B Coder MTP Q5_K_M on http://127.0.0.1:$Port/v1"
